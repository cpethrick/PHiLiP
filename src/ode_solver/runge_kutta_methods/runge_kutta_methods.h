...
/// 12-stage, 2nd-order P-ERK scheme for spatial order p=5
#if PHILIP_DIM==1
template <int dim, typename real, typename MeshType = dealii::Triangulation<dim>>
#else
template <int dim, typename real, typename MeshType = dealii::parallel::distributed::Triangulation<dim>>
#endif
class PERK_2_5_12: public RKTableauBase <dim, real, MeshType>
{
public:
    /// Constructor
    PERK_2_5_12(const int n_rk_stages, const std::string rk_method_string_input) 
        : RKTableauBase<dim,real,MeshType>(n_rk_stages, rk_method_string_input) { }
...
/// 12-stage, 2nd-order P-ERK scheme for spatial order p=4
#if PHILIP_DIM==1
template <int dim, typename real, typename MeshType = dealii::Triangulation<dim>>
#else
template <int dim, typename real, typename MeshType = dealii::parallel::distributed::Triangulation<dim>>
#endif
class PERK_2_4_12: public RKTableauBase <dim, real, MeshType>
{
public:
    /// Constructor
    PERK_2_4_12(const int n_rk_stages, const std::string rk_method_string_input) 
        : RKTableauBase<dim,real,MeshType>(n_rk_stages, rk_method_string_input) { }
