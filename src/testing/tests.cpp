...
::select_test(const AllParam *const parameters_input,
    ...
    } else if(test_type == Test_enum::circular_cylinder_flow) {
        if constexpr (dim==2 && nstate==dim+2)  return std::make_unique<CircularCylinderFlow<dim, nstate>>(parameters_input, parameter_handler_input);
    } else {
