...
#include "flow_solver_cases/tandem_spheres_flow.h"
                   ...
                   const dealii::ParameterHandler &parameter_handler_input)
{
    ...
    if (flow_type == FlowCaseEnum::taylor_green_vortex){
        ...
        if constexpr ((dim==3 || dim == 2) && nstate==dim+2){
            std::shared_ptr<FlowSolverCaseBase<dim, nstate>> flow_solver_case = std::make_shared<TandemSpheresFlow<dim, nstate>>(parameters_input);
            return std::make_unique<FlowSolver<dim, nstate>>(parameters_input, flow_solver_case, parameter_handler_input);
        }
    } else {
