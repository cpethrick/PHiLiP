...
/// Manufactured solution corresponding to Friedrich et al 2019
template <int dim, typename real>
class ManufacturedSolutionEulerSpacetime 
    : public ManufacturedSolutionFunction<dim, real>
{
// We want the Point to be templated on the type,
// however, dealii does not template that part of the Function.
...
public:
    /// Constructor
    explicit ManufacturedSolutionEulerSpacetime(const unsigned int nstate = 1)
        :   ManufacturedSolutionFunction<dim,real>(nstate){}
    /// Value
    real value (const dealii::Point<dim,real> &point, const unsigned int istate = 0) const override;
    /// Gradient
