...
#include "straight_semiperiodic_cube.hpp"
                            ...
                            const int number_of_cells_per_direction)
{
    ...
    if constexpr(dim==2) {
        ...
        grid_type_string = "Cube, periodic on faces (0 and 1) and (2 and 3).";
        std::vector<dealii::GridTools::PeriodicFacePair<typename dealii::Triangulation<dim>::cell_iterator> > matched_pairs;
        dealii::GridTools::collect_periodic_faces(*grid,0,1,0,matched_pairs);
...
// Not to be used for dim == 1 
#if PHILIP_DIM!=1
    template void straight_semiperiodic_cube<PHILIP_DIM, dealii::parallel::distributed::Triangulation<PHILIP_DIM>> (std::shared_ptr<dealii::parallel::distributed::Triangulation<PHILIP_DIM>> &grid, const double domain_left, const double domain_right, const int number_of_cells_per_direction);
#endif
